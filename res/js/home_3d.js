...
//    this.t = this.element.offsetTop;
  ...
  Node.prototype._bindEvents = function _bindEvents() {
    ...
    this.element.addEventListener('mouseout', this._mouseOutHandler, false);
      ...
      this.element.parentNode.addEventListener('mouseenter', this._mouseEnterHandler, false);
      this.element.parentNode.addEventListener('mouseout', this._mouseOutHandler, false);

      $(".box-events")[0].addEventListener('mouseout mouseenter', function(e) {
      }, true);

      $(".e-h3")[0].addEventListener('mouseout mouseenter', function() {
        ele.parentNode.removeEventListener('mouseenter', that._mouseEnterHandler, false);
        ele.parentNode.removeEventListener('mouseout', that._mouseOutHandler, false)
      });*/
  ...
  Node.prototype._addClass = function _addClass(ev, state) {
    ...
    switch (direction) {
      case 0:
        class_suffix = '-top';self_Z = $(this.element).find(".w").height() / 2;break;
      case 1:
        class_suffix = '-right';self_Z = $(this.element).find(".w").width() / 2;break;
      case 2:
        class_suffix = '-bottom';self_Z = $(this.element).find(".w").height() / 2;break;
      case 3:
        class_suffix = '-left';self_Z = $(this.element).find(".w").width() / 2;break;
    }

    this.element.className = '';
    this.element.classList.add(state + class_suffix);

     $(this.element).find(".w").css({
          "transformOrigin" : "50% 50% " + (-self_Z) + "px",
          "webkitTransformOrigin" : "50% 50% " + (-self_Z) + "px",
          "mozTransformOrigin" : "50% 50% " + (-self_Z) + "px"
      });
...
new DAH(document.querySelectorAll('.main-demo li'));